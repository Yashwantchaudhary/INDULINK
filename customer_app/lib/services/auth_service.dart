...
class AuthService {
  ...
  Future<Map<String, dynamic>> register({
    ...
    try {
      ...
      developer.log('Registration data: firstName=$firstName, lastName=$lastName, email=$email, role=$role', name: 'AuthService');
      ...
      developer.log('Registration response status: ${response.statusCode}', name: 'AuthService');
      developer.log('Registration response data: ${response.data}', name: 'AuthService');
      ...
      if (response.statusCode == 201 && response.data['success'] == true) {
        ...
        developer.log('Registered user role: ${user.role}', name: 'AuthService');
        return {'success': true, 'user': user};
      } else {
        developer.log('Registration failed: ${response.data}', name: 'AuthService');
        return {
          'success': false,
      ...
      developer.log('Registration network error: $e', name: 'AuthService', error: e);
      ...
      if (e is DioException) {
        ...
        if (e.response != null) {
          developer.log('Dio response status: ${e.response?.statusCode}', name: 'AuthService');
          developer.log('Dio response data: ${e.response?.data}', name: 'AuthService');
        }
      }
      return {
        'success': false,
  ...
  Future<Map<String, dynamic>> login({
    ...
    try {
      ...
      developer.log('Login response received: ${response.statusCode}', name: 'AuthService');
  ...
  Future<Map<String, dynamic>> refreshToken() async {
    try {
      ...
      if (response.statusCode == 200 && response.data['success'] == true) {
        final accessToken = response.data['data']['accessToken'];
        await _saveTokens(accessToken, accessToken); // Note: refresh endpoint only returns new access token
        
        developer.log('Token refresh successful', name: 'AuthService');
        return {'success': true, 'accessToken': accessToken};
      } else {
        developer.log('Token refresh failed: ${response.data}', name: 'AuthService');
  ...
  Future<Map<String, dynamic>> forgotPassword(String email) async {
    try {
      developer.log('Requesting password reset for: $email', name: 'AuthService');
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data['success'] == true) {
        ...
        if (currentUser != null) {
          ...
          return {'success': true, 'user': currentUser, 'profileImage': profileImageUrl};
        } else {
        ...
        return {
          'success': false,
          'message': response.data['message'] ?? 'Failed to upload profile image'
        };
      }
    } catch (e) {
      developer.log('Profile image upload error: $e', name: 'AuthService', error: e);
